...
static int drawstatusbar(Monitor *m, int bh, char* text, unsigned int stw);
...
int
drawstatusbar(Monitor *m, int bh, char* stext, unsigned int stw) {
	int ret, i, w, x, len;
	short isCode = 0;
	char *text;
	char *p;
...
grabkeys(void)
{
	updatenumlockmask();
	{
		...
		for (i = 0; i < LENGTH(keys); i++) {
            ...
            for (j = 0; j < LENGTH(modifiers); j++)
                XGrabKey(dpy, keycode, keys[i].mod | modifiers[j], root,
...
resizeclient(Client *c, int x, int y, int w, int h)
{
    ...
    char isOnlyWindow = (nexttiled(c->mon->clients) == c) && !nexttiled(c->next);

    if (isOnlyWindow)
	    resetlayout(NULL);

	if ((isOnlyWindow || &monocle == c->mon->lt[c->mon->sellt]->arrange)
